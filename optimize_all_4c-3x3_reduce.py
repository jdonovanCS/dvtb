...
import sys
sys.path.append('/p/home/rditljtd/caffe/python/')
...
path = '/p/work/rditljtd/deep-visualization-toolbox/models/minc-googlenet/'
imagepath = '/p/work/rditljtd/Material\ Classification/minc-model/'
if not os.path.exists(imagepath + 'images'):
...
print(path+'deploy-{}2.prototxt'.format(arch), path + 'minc-{}.caffemodel'.format(arch))
net1=caffe.Classifier(path + 'deploy-{}2.prototxt'.format(arch), path + 'minc-{}.caffemodel'.format(arch),channel_swap=(2,1,0),mean=numpy.array([104,117,124]))
...
for layer_name in net1._layer_names:
    if 'Conv' not in layers[list(net1._layer_names).index(layer_name)].type: #and 'Pool' not in layers[list(net_for_optimization._layer_names).index(layer_name)].type:
        continue
    if (layer_name != 'inception_4c/3x3_reduce'):
        continue
    node_count = 0
