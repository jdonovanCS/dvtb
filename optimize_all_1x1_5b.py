...
for layer_name in net1._layer_names:
    # if 'Conv' not in layers[list(net1._layer_names).index(layer_name)].type: #and 'Pool' not in layers[list(net_for_optimization._layer_names).index(layer_name)].type:
    #     continue
    # if (layer_name in ['conv1/7x7_s2', 'conv2/3x3_reduce', 'conv2/3x3', 'inception_3a/1x1', 'inception_3a/3x3', 'inception_3a/3x3_reduce', 'inception_3a/5x5_reduce', 'inception_3a/5x5', 'inception_3a/pool_proj',
    #                     'inception_3b/1x1', 'inception_3b/3x3_reduce', 'inception_3b/3x3', 'inception_3b/5x5', 'inception_3b/5x5_reduce', 'inception_3b/pool_proj', 'inception_4a/1x1', 'inception_4a/3x3_reduce',
    #                     'inception_4a/3x3', 'inception_4a/5x5_reduce', 'inception_4a/5x5', 'inception_4a/pool_proj', 'inception_4b/1x1', 'inception_4b/3x3_reduce', 'inception_4b/3x3', 'inception_4b/5x5_reduce',
    #                     'inception_4b/5x5', 'inception_4b/pool_proj', 'inception_4c/3x3_reduce', 'inception_4d/1x1'] or '1x1' not in layer_name or 'reduce' in layer_name or layer_name != 'inception_5b/1x1'):
    #     continue
    if layer_name != 'conv1/7x7_s2':
        continue
    node_count = 0
    if layer_name == 'inception_5b/1x1':
        node_count = 304 #node_count = 332 #check this prolly wanna start at 304 to 330
    
    for node in (net1.blobs[layer_name].data[0]):
        if node_count == len(net1.blobs[layer_name].data[0]) or node_count==335:
            break
        command = construct_command(None, None, None, None, None, args.data_size, None, args.rand_seed, 
                    layer_name, node_count, args.push_spatial, args.push_dir, args.decay,
